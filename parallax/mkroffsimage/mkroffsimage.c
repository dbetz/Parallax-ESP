...
#else
    if (!(fdat = malloc(size))) {
        perror("allocate file buffer");
        return 0;
    }
    lseek(f, 0, SEEK_SET);
    if (read(f, fdat, size) != size) {
        perror("read entire file");
        return 0;
    }
#endif
